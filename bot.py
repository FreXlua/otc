...
# Конфигурация бота
BOT_TOKEN = "7721007358:AAGNR2WFCwUNLLAnNzrRFGSlw8_F16Zos9w"  # Замените на ваш токен
ADMIN_ID = 1799193124 # ID администратора
VALUTE = "TON"  # По умолчанию валюта - TON
supp0rt = "https://forms.gle/4kN2r57SJiPrxBjf9" # поддержка бота
bot_username = "GiftElfRfbot" # юзер бота
# Хранение данных
    ...
    try:
        ...
        if user_id == ADMIN_ID:
            ...
            keyboard = [
              ...
              #  [InlineKeyboardButton(get_text(lang, "support_button"), url=f'https://t.me/{supp0rt}')],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if user_id == ADMIN_ID and admin_commands.get(user_id) == 'change_balance':
            ...
            await update.message.reply_text(
                get_text(lang, "deal_created_message", amount=deals[deal_id]['amount'], valute=VALUTE, description=deals[deal_id]['description'], deal_link=f"https://t.me/{bot_username}?start={deal_id}"),
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(get_text(lang, "menu_button"), callback_data='menu')]])
            )
            # Уведомление админу
            await context.bot.send_message(
